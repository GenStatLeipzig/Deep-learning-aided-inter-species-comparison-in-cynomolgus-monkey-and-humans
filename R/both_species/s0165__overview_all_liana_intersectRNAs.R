...
gzfiles_human_intersect  = dir(here("J:/02_projekte/2403_scRNAseq_cross_species_primate_human/github/scRNAseq_cross_species_primate_human_km/analysis/results/new_29_01_25/H26_cyno_genes/"),pattern = "*iana_res.txt.gz", full.names = T)
gzfiles_human_intersect

gzfiles_cyno_intersect  = dir(here("J:/02_projekte/2403_scRNAseq_cross_species_primate_human/github/scRNAseq_cross_species_primate_human_km/analysis/results/new_29_01_25/M26_cyno_genes/"),pattern = "*iana_res.txt.gz", full.names = T)
gzfiles_cyno_intersect


all = lapply(c(gzfiles_human_intersect, gzfiles_cyno_intersect) , function(myfile) {
  # myfile =  gzfiles[1]
  resi = fread(myfile)
  resi[, fn := myfile]
  resi
...
all[, .N, csv]
all[, species := fifelse(grepl("^H",csv), "H.sapiens", "M.fascicularis")]
all[, .N,species]
...
p_count_all = p_targets / p_sources + plot_layout(guides = "collect") + plot_annotation(tag_suffix =  ")", tag_levels = "A", title = "All Sources-Receptor-Interactions", caption = "J:/02_projekte/2210_snRNA_Sympath_hi_maus_herz/scripts/s0165_magnitude_targets_sources_grouped_intersectRNAs.pdf")  &
...
pdf(here("analysisR/results/s0165_magnitude_targets_sources_grouped_intersectRNAs.pdf"), width = 11, height = 11)
p_count_all
dev.off()

pdf(here("analysisR/results_GIT/s0165_magnitude_targets_sources_grouped_intersectRNAs.pdf"), width = 11, height = 11)
p_count_all
dev.off()
#
#
fwrite(all2_source, here("analysisR/results/s0165_magnitude_targets_grouped_all2_source_intersectRNAs.csv"))
fwrite(all2_target, here("analysisR/results/s0165_magnitude_targets_grouped_all2_targets_intersectRNAs.csv"))

fwrite(all2_source, here("analysisR/results_GIT/s0165_magnitude_targets_grouped_all2_source_intersectRNAs.csv"))
fwrite(all2_target, here("analysisR/results_GIT/s0165_magnitude_targets_grouped_all2_targets_intersectRNAs.csv"))
#
